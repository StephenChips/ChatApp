...
export function AddContactDialog() {
      ...
      <DialogContent>
        <Box component="form" sx={{ marginTop: 1 }}>
          ...
          <Box display="flex" alignItems="center" sx={{ marginTop: 2 }}>
            <Typography variant="h4" sx={{ color: "gray", marginRight: 2 }}>#</Typography>
            <Tooltip
          ...
          <Box sx={{ marginTop: 2 }} display="flex" justifyContent="space-between" width="100%">
            ...
            <Button variant="contained" onClick={onClickSearchUserButton}>Search User</Button>
          </Box>
        </Box>
        <Collapse in={isSearchResultVisible} sx={{ marginTop: 2 }}>
          <Divider>{searchResult.status === "searching" ? "Searching" : "Search Result"}</Divider>
                  ...
                  if (searchResult.user) {
                    ...
                    return (
                      <Box display="flex" flexGrow={1} alignItems="center">
                        <img style={{ width: "40px", height: "40px", borderRadius: "20px" }} src={avatarURL} />
                        <Box marginLeft={2} marginRight="auto">
                          <Box fontSize="18px" fontWeight="bold">{name}</Box>
                    ...
                    return <>The user you searched doesn't exist. (ChatApp ID: {textFieldValue}) </>
                  }
                } else {
                  return  (
                    <>
                      <Typography>Somthing went wrong, cannot search the user successfully.</Typography>
  ...
  function onChangeTextField(e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) {
